...
# Calculate appropriate pool size based on environment
# Default pool settings can be overridden via environment variables
POOL_SIZE = int(os.getenv("DB_POOL_SIZE", "5"))  # Reduced from 10 to 5
MAX_OVERFLOW = int(os.getenv("DB_MAX_OVERFLOW", "10"))  # Reduced from 20 to 10
# Total max connections per worker: POOL_SIZE + MAX_OVERFLOW = 15 (was 30)
# With 4 workers: 60 connections per Cloud Run instance (was 120)

engine = create_engine(
    SQLALCHEMY_DATABASE_URL,
    # Connection pool settings optimized for Cloud SQL with idle timeout handling
    pool_size=POOL_SIZE,
    ...
    # Aggressive connection validation and keepalive settings
    connect_args={
        "connect_timeout": 10,
        "application_name": "rhesis-backend",
        "keepalives": 1,  # Enable TCP keepalives
        "keepalives_idle": 30,  # Start keepalive after 30s idle (was 300s)
