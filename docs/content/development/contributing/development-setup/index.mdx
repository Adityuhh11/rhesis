...
We use [uv](https://docs.astral.sh/uv/) as our Python package manage<CodeBlock filename="Terminal" language="bash">
{`# Install uv
curl -LsSf https://astral.sh/uv/install.sh | sh

# Verify installation
uv --version`}
</CodeBlock>tall.sh | sh
...
We recommend using [nvm](https://gith<CodeBlock filename="Terminal" language="bash">
{`# Install nvm
...
node --version
npm -<CodeBlock filename="Terminal" language="bash">
...
# C<CodeBlock filename="Terminal" language="bash">
{`# macOS (using Homebrew)
brew install redis
...
</CodeBlock>upstream https://github.com/rhe<CodeBlock filename="Terminal" language="bash">
...
# Return to r<CodeBlock filename="Terminal" language="bash">
{`# Copy the example environment file
cp .env.example .env

# Edit the environment file with your configuration
# You'll need to set up database URLs, API keys, etc.`}
</CodeBlock>'ll need to set up database URLs, <CodeBlock filename="Terminal" language="bash">
...
# Return to root
cd ../..`}
</CodeBlock>s.create_initial_data
...
We pr<CodeBlock filename="Terminal" language="bash">
{`# Start all services
./rh backend start
./rh frontend start
./rh docs start
...
./rh start all`}
</CodeBlock> them all at once (if configured)
./rh start all
```

### Manual <CodeBlock filename="Terminal" language="bash">
{`# Backend (FastAPI)
cd apps/backend
...
</CodeBlock>n celery -A rhesis.worker worker --loglevel=info
```
...
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
  ...
  <div>
    <h3>Redis</h3>
    <p>Verify Redis connection for background<CodeBlock filename="Terminal" language="bash">
{`# Test backend health
...
</CodeBlock>ort redis; r = redis.Redis(); print('Redis OK')"
```
...
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
  ...
  <div>
    ...
    <p>Automated checks before commits</p<CodeBlock filename="Terminal" language="bash">
{`# Install pre-commit hooks
uvx pre-commit install

# Run pre-commit on all files
uvx pre-commit run --all-files`}
</CodeBlock>mmit on all files
uvx pre-commit run --all-files
```
...
# All tests from root
uv<CodeBlock filename="example.py" language="python">
...
</CodeBlock>   assert response.json()["status"] == "healthy"
```
...
<Callout type="default">
  **Port Conflicts** If you get port conflicts, check what's running on the
  ...
  - Redis<CodeBlock filename="Terminal" language="bash">
...
</CodeBlock>psql -h localhost -U your_username<CodeBlock filename="Terminal" language="bash">
...
# Check Python<CodeBlock filename="Terminal" language="bash">
{`# Clear npm cache
...
<Callout type="default">
  **Need Help?** If you encounter any issues during setup, don't hesitate to: -
  Check our [Troubleshooting Guide](/docs/troubleshooting) - Ask questions in
