...
</CodeBlock>com/rhesis-ai/rhesis.git
cd rhe<CodeBlock filename="Terminal" language="bash">
{`npm install
# or
yarn install`}
</CodeBlock>endencies
...
Copy the example<CodeBlock filename="Terminal" language="bash">
{`cp .env.example .env.local`}
</CodeBlock>ired values:`}
</CodeBlock>bash
cp .env.example .env.local
<CodeBlock filename="Terminal" language="bash">
{`
...
- `AUTH_SECRET`: Should be the same as `NE<CodeBlock filename="Terminal" language="bash">
{`npm run dev`}
</CodeBlock> Run the Development Server`}
</CodeBlock>bash
npm run dev
<CodeBlock filename="Terminal" language="bash">
{`
...
Create<CodeBlock filename="example.json" language="json">
{`{
...
</CodeBlock>typescript.enablePromptUseWorkspaceTsdk": true
}`}
</CodeBlock>
