...
The following environment vari<CodeBlock filename="Terminal" language="bash">
...
The system wo<CodeBlock filename="Terminal" language="bash">
{`# Gmail SMTP
...
The new decorator approach provi<CodeBlock filename="example.python" language="python">
...
</CodeBlock> "success", "test_run_id": "option<CodeBlock filename="example.python" language="python">
{`from rhesis.backend.notifications.email.template_service import EmailTemplate
...
</CodeBlock>n(template=EmailTemplate.TEST_EXECUTION_SUMMARY)
<CodeBlock filename="example.text" language="text">
{`
#### Custom Subject Templates

You can cus<CodeBlock filename="example.python" language="python">
{`@email_notification(
    template=EmailTemplate.TEST_EXECUTION_SUMMARY,
    subject_template="Test Execution Complete: {test_set_name} - {status.title()}"
)`}
</CodeBlock> Complete: {test_set_name} - {status.title()}"
)`}
</CodeBlock>
...
Use the decorator for tasks that us<CodeBlock filename="example.python" language="python">
{`from rhesis.backend.tasks.base import BaseTask, with_tenant_context, email_notification
from rhesis.backend.notifications.email.template_service import EmailTemplate
from rhesis.backend.worker import app
...
def user_facing_task(self, params, db=None):
    ...
    # Your task logic here
    return {"result": "success", "test_run_id": "optional-for-links"}`}
</CodeBlock> "success", "test_run_id": "optional-for-links"}
<CodeBlock filename="example.python" language="python">
{`
#### 2. **Silent Tasks** - No Decorator

Tasks withou<CodeBlock filename="example.python" language="python">
...
</CodeBlock>task logic here
    return {"result": "success"}`}
</CodeBlock>

#### 3. **Legacy EmailEnabledTask** - Still Supported

The old `EmailEnabledTa<CodeBlock filename="example.python" language="python">
...
When running tasks in para<CodeBlock filename="example.python" language="python">
...
Additional variable<CodeBlock filename="example.python" language="python">
{`@email_notification(template=EmailTemplate.TEST_EXECUTION_SUMMARY)
@app.task(base=BaseTask, bind=True)
def test_task(self):
    # Task logic here
    return {
        ...
        'project_n<CodeBlock filename="example.python" language="python">
{`from rhesis.backend.tasks.base import BaseTask, with_tenant_context, email_notification
from rhesis.backend.notifications.email.template_service import EmailTemplate
from rhesis.backend.worker import app
...
The core `EmailS<CodeBlock filename="example.python" language="python">
{`class EmailService:
    def __init__(self):
...
Us<CodeBlock filename="Terminal" language="bash">
...
{
  "message": "Test<CodeBlock filename="example.python" language="python">
...
#### No Emai<CodeBlock filename="Terminal" language="bash">
{`   # In worker logs, look for:
   "SMTP configuration incomplete. Email notifications will be disabled."`}
</CodeBlock>plete. Email notifications will be disabled<CodeBlock filename="Terminal" language="bash">
{`   kubectl exec -it deployment/rhesis-worker -- env | grep SMTP`}
</CodeBlock>t deployment/rhesis-worker -- env | grep SMTP`}
</CodeBlock>
...
Monitor em<CodeBlock filename="Terminal" language="bash">
{`   kubectl logs deployment/rhesis-worker | grep "email"`}
</CodeBlock> logs deployment/rhesis-worker | grep "email"
   <CodeBlock filename="example.text" language="text">
{`
...
Ensure your worker d<CodeBlock filename="example.yaml" language="yaml">
  ...
  - name: FRONTEND_URL
    valueFrom:
      secretKeyRef:
        name: rhesis-worker-secrets
        key: FRONTEND_URL`}
</CodeBlock> rhesis-worker-secrets
        key: FRONTEND_URL`}
</CodeBlock>

### CI/CD Pipeline

The GitHub Act<CodeBlock filename="example.yaml" language="yaml">
...
</CodeBlock>ral=FRONTEND_URL="${{ secrets.FRONTEND_URL }}" \
```
