...
All CRUD operations accept `org<CodeBlock filename="example.py" language="python">
{`def create_entity(
    db: Session,
...
Database queries inclu<CodeBlock filename="example.py" language="python">
...
Database s<CodeBlock filename="example.py" language="python">
{`@contextmanager
...
FastAPI dependencies <CodeBlock filename="example.py" language="python">
...
### Usage Patter<CodeBlock filename="example.py" language="python">
...
</CodeBlock>tomatic commit on success, rollba<CodeBlock filename="example.py" language="python">
...
The multi-tenancy system integrates with authentication to s<CodeBlock filename="example.py" language="python">
{`async def require_current_user(request: Request, db: Session = Depends(get_db)):
    ...
    return user`}
</CodeBlock>nization_id"), user.get("sub"))
...
Superusers can access data across organizat<CodeBlock filename="example.py" language="python">
{`def bypass_rls(db: Session):
    """Temporarily bypass row-level security for superuser operations."""
    db.execute(text("SET app.bypass_rls = true"))`}
</CodeBlock>   db.execute(text("SET app.bypass_rls = true"))
```
