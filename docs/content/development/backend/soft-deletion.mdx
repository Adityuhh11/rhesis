...
New metho<CodeBlock filename="example.python" language="python">
...
Use the `without_soft_delete_filter()`<CodeBlock filename="example.python" language="python">
...
The CRUD util<CodeBlock filename="example.python" language="python">
...
</CodeBlock>t_item(db, Model, item_id, include_deleted=True)`}
</CodeBlock>
...
**Result:** Both count queries and paginated results correctly exclude deleted records, providing accurat<CodeBlock filename="example.python" language="python">
...
</CodeBlock>LECT COUNT(*) FROM user WHERE deleted_at IS NULL
<CodeBlock filename="Terminal" language="bash">
{`
## Database Migration

The migration<CodeBlock filename="Terminal" language="bash">
{`# Run migration
cd apps/backend
alembic upgrade head`}
</CodeBlock>n migration
cd apps/backend
alembic upgrade head`}
</CodeBlock>
...
</CodeBlock>dels.Test, test_id,
    or<CodeBlock filename="example.python" language="python">
...
deleted_tests = QueryBuilder(db, models.Test)\
    ...
    .all()`}
</CodeBlock> .with_sorting('deleted_at', <CodeBlock filename="example.python" language="python">
{`from rhesis.backend.app.database import without_soft_delete_filter
...
with without_soft_delete_filter():
    ...
    deleted_count = db.query(models.User)\
        ...
        .count()`}
</CodeBlock>s.User.deleted_at.isnot(None))\
        .count()
```
