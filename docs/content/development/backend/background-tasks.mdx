...
Celery work<CodeBlock filename="example.dockerfile" language="dockerfile">
...
</CodeBlock>RKER_OPTS="--without-heartbeat --without-gossip"
...
celery -A rhesis.backend.worker.app worker \
    ...
    \${CELERY_WORKER_OPTS}`}
</CodeBlock><CodeBlock filename="Terminal" language="bash">
...
The worker.py f<CodeBlock filename="example.python" language="python">
{`app.conf.update(
    # Other settings...
...
</CodeBlock>amqp result store
    result_persistent=False,
)`}
</CodeBlock>

Additionally, the results handling in `tasks/execution/results.py` in<CodeBlock filename="example.python" language="python">
...
</CodeBlock>_tasks < total_tests else RunStatus.FAILED.value
<CodeBlock filename="example.text" language="text">
{`
#### Purging Stuck Tasks

If you encounter st<CodeBlock filename="Terminal" language="bash">
...
For a more targeted ap<CodeBlock filename="Terminal" language="bash">
...
</CodeBlock>hesis.backend.worker control terminate <task_id>
<CodeBlock filename="example.text" language="text">
{`
...
The `execute_single_test` task in `tasks/execution/test.py<CodeBlock filename="example.python" language="python">
