...
Before the cascade syste<CodeBlock filename="example.python" language="python">
...
With the ca<CodeBlock filename="example.python" language="python">
...
</CodeBlock>b, models.TestRun, test_run_id, org_id, user_id)`}
</CodeBlock>
...
Centralized reg<CodeBlock filename="example.python" language="python">
...
Generic service that reads the c<CodeBlock filename="example.python" language="python">
...
The generic `delete_item()` and `restore_item()`<CodeBlock filename="example.python" language="python">
{`def delete_item(
    db: Session,
    model: Type[T],
    item_id: uuid.UUID,
...
Add a new entry or appen<CodeBlock filename="example.python" language="python">
{`CASCADE_RELATIONSHIPS: Dict[Type, List[CascadeRelationship]] = {
...
</CodeBlock>tadata belongs to a test set"
        )
    ],
}`}
</CodeBlock>
...
That's it! No code changes nee<CodeBlock filename="example.python" language="python">
...
</CodeBlock>          assert test_set.deleted_at is not None
<CodeBlock filename="example.text" language="text">
{`
## Advanced Configuration

### Disable Delete or Restore for Specific Relationship<CodeBlock filename="example.python" language="python">
{`CascadeRelationship(
...
</CodeBlock>scription="Audit logs should never be deleted"
)`}
</CodeBlock>

### Complex Multi-Level Cascades

The <CodeBlock filename="example.python" language="python">
...
</CodeBlock>odel=models.TestResult, foreign_key='test_id')
]
<CodeBlock filename="example.sql" language="sql">
{`
...
The cascade service uses efficie<CodeBlock filename="example.python" language="python">
...
The cascade service automati<CodeBlock filename="example.python" language="python">
{`if organization_id and hasattr(rel.child_model, 'organization_id'):
    query = query.filter(rel.child_model.organization_id == organization_id)`}
</CodeBlock>.child_model.organization_id == organization_id)
<CodeBlock filename="example.sql" language="sql">
{`
This ensures:
...
## Transaction Managemen<CodeBlock filename="example.python" language="python">
{`try:
...
### Un<CodeBlock filename="example.python" language="python">
{`@pytest.mark.unit
@pytest.mark.crud
...
</CodeBlock>            assert result.deleted_at is not None`}
</CodeBlock>

### Test Data Factori<CodeBlock filename="example.python" language="python">
{`@pytest.fixture
...
Al<CodeBlock filename="example.python" language="python">
{`CascadeRelationship(
    child_model=models.TestResult,
    foreign_key='test_run_id',
    description="Test results belong to a test run and should cascade with it"
)`}
</CodeBlock>long to a test run and should cascade with it"
)`}
</CodeBlock>

### 2. Consider Data Preser<CodeBlock filename="example.python" language="python">
...
</CodeBlock> should be removed when user is deleted"
    )
]
<CodeBlock filename="example.python" language="python">
{`
...
Alway<CodeBlock filename="example.python" language="python">
{`# tests/backend/crud/test_project_cascade_delete.py
def test_delete_project_cascades_to_test_sets(test_db, db_project):
    # ... test implementation ...`}
</CodeBlock>, db_project):
    # ... test implementation ...`}
</CodeBlock>

### 4. Monitor Performance

For entities with m<CodeBlock filename="example.python" language="python">
...
</CodeBlock>cords for {parent_model.__name__} {parent_id}"
)
<CodeBlock filename="example.text" language="text">
{`
### 5. Keep Cascade Chains Short

Avoid deep cascade chain<CodeBlock filename="example.python" language="python">
...
</CodeBlock>tion -> Project -> TestSet -> Test -> TestResult`}
</CodeBlock>

## Common Patterns

### Patte<CodeBlock filename="example.python" language="python">
{`models.TestRun: [
    CascadeRelationship(
        child_model=models.TestResult,
        foreign_key='test_run_id'
...
<CodeBlock filename="example.text" language="text">
{`
### Pattern 2: Mult<CodeBlock filename="example.python" language="python">
{`models.Test: [
    CascadeRelationship(
        child_model=models.TestResult,
        foreign_key='test_id'
...
### Pattern 3: Conditional C<CodeBlock filename="example.python" language="python">
{`models.Organization: [
    CascadeRelationship(
...
</CodeBlock>on't auto-restore (manual review needed)
    )
]
<CodeBlock filename="example.sql" language="sql">
{`
...
#### Step 1: Add Configuration<CodeBlock filename="example.python" language="python">
{`models.YourParent: [
    CascadeRelationship(
...
#### Step 2:<CodeBlock filename="example.python" language="python">
...
</CodeBlock>, models.YourParent, parent_id, org_id, user_id)
<CodeBlock filename="example.python" language="python">
{`
#### Step 3: <CodeBlock filename="example.python" language="python">
