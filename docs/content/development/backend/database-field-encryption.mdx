...
- Battle-te<CodeBlock filename="Terminal" language="bash">
{`cd apps/backend
uv add cryptography`}
</CodeBlock>:**

```bash
cd apps<CodeBlock filename="example.python" language="python">
...
</CodeBlock>crypted).decode()  # Returns: "my-api-key-12345"
...
**Key Generat<CodeBlock filename="Terminal" language="bash">
{`python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`}
</CodeBlock>t Fernet; print(Fernet.generate_key().decode())"`}
</CodeBlock>
...
The `EncryptedString` SQLAlchemy TypeDecorator will gracefully handle both encrypted<CodeBlock filename="example.python" language="python">
...
</CodeBlock>alue  # Return plaintext during migration window
<CodeBlock filename="example.python" language="python">
{`
...
Encryption/decryption will be transparent to ap<CodeBlock filename="example.python" language="python">
...
</CodeBlock>urn self.cipher.decrypt(<CodeBlock filename="example.python" language="python">
...
</CodeBlock>lumn(EncryptedString)  # Transparently encrypted`}
</CodeBlock>
...
**1. Key Rotation:**
Implement support for dual-ke<CodeBlock filename="example.python" language="python">
...
</CodeBlock>(OLD_KEY).decrypt(value)  # Fall back to old key
```
