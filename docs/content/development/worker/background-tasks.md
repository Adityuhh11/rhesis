...
Th<CodeBlock filename="example.python" language="python">
{`import os
...
</CodeBlock>over_tasks(["rhesis.backend.tasks"], force=True)
<CodeBlock filename="example.text" language="text">
{`
The application uses Redis as bo<CodeBlock filename="Terminal" language="bash">
...
All tasks inherit from a `BaseTask` class that provides retry logic, error handling, and most important<CodeBlock filename="example.python" language="python">
...
</CodeBlock>       self.request.user_id = headers['user_id']
...
A task decorator is provided to automatically ha<CodeBlock filename="example.python" language="python">
...
def with_tenant_context(func):
    ...
    return wrapper`}
</CodeBlock> func(self, *args, **kwargs)

    return wrapper`}
</CodeBlock>

Using this decorator sim<CodeBlock filename="example.python" language="python">
...
A `task_launcher` utility method is provided to easily lau<CodeBlock filename="example.python" language="python">
...
When creating a task, you no longer need to explicitly require organization_id and user_id as parameters. The context system handles this autom<CodeBlock filename="example.python" language="python">
...
</CodeBlock>ssage}, Organization: {org_id}, User: {user_<CodeBlock filename="example.python" language="python">
...
</CodeBlock>r(test_config.id) if test_config else None
    }
<CodeBlock filename="example.python" language="python">
...
def manual_db_example():
    ...
    return results`}
</CodeBlock>set_count"] = len(test_sets)
...
The most common w<CodeBlock filename="example.python" language="python">
...
</CodeBlock>g status later
    return {"task_id": result.id}`}
</CodeBlock>

## Worker Configuration

Celery workers are con<CodeBlock filename="example.dockerfile" language="dockerfile">
...
ENV CELERY_WORKER_CONCURRENCY=8 \
    ...
    CELERY_WORKER_OPTS=""`}
</CodeBlock>WORKER_LOGLEVEL=INFO \
    CELERY_WORKER_OPTS=""
<CodeBlock filename="example.text" language="text">
{`
...
</CodeBlock>        'socket_timeout': 30,
        }
    },
)`}
</CodeBlock>

The w<CodeBlock filename="Terminal" language="bash">
...
celery -A rhesis.backend.worker.app worker \
    ...
    ${CELERY_WORKER_OPTS}`}
</CodeBlock>SKS_PER_CHILD:-1000} \
    ${CELERY_WORKER_OPTS}
```<CodeBlock filename="Terminal" language="bash">
{`# Enable Flower monitoring
docker run -e ENABLE_FLOWER=yes -p 5555:5555 your-worker-image`}
</CodeBlock>ENABLE_FLOWER=yes -p 5555:5555 your-worker-image
<CodeBlock filename="example.python" language="python">
{`
## Task Monitoring

Task status can be monit<CodeBlock filename="example.python" language="python">
...
</CodeBlock>esult.error) if result.failed() else None,
    }`}
</CodeBlock>
