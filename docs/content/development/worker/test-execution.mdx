...
The system uses the **Strategy Pattern** to route t<CodeBlock filename="example.text" language="text">
...
3. **Shared Utilities** (`executors/shared.py`)
   ...
   - Result <CodeBlock filename="Diagram" language="text">
{`Test Configuration
    ↓
Retrieve Test
    ↓
...
All executor<CodeBlock filename="example.py" language="python">
...
- `metrics` (Dict[str, Any]): Metric evaluat<CodeBlock filename="example.py" language="python">
...
print(f"Metrics: {resu<CodeBlock filename="example.py" language="python">
...
</CodeBlock>sult = executor.execute(db, test_config_id, ...)
```
...
class ImageTestExecutor(BaseTestExecutor):
    def execute(self, ...):
        ...
        pass`}
</CodeBlock>        # Image-specific test<CodeBlock filename="example.py" language="python">
...
The test execution code is located in the backen<CodeBlock filename="Diagram" language="text">
...
</CodeBlock>get.py             # Backend target for Penelope
```
