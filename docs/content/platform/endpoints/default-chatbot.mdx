...
<CodeBlock filename="Headers" isTerminal={true}>
...
<CodeBlock filename="api-info-auth-response.json" language="json">
  {`{
  "service": "Rhesis Insurance Chatbot API",
  "description": "Chat with Rosalind, your insurance expert",
  "endpoints": {
...
The chatbot supports bearer token authentication for secure access. The authentication mode is automatically configured based on environment settings (see [Deployment Modes](#deployment-modes) for details):
...
<CodeBlock filename="Authorization Header" isTerminal={true}>
...
<CodeBlock filename="error-response.json" language="json">
{`{
...
<CodeBlock filename="public-endpoint-config.json" language="json">
...
<CodeBlock filename="authenticated-endpoint-config.json" language="json">
{`// Endpoint configuration in backend
{
  "name": "Insurance Chatbot",
  "url": "https://chatbot.rhesis.ai/chat",
